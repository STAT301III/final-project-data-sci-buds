...
## Best Model Fitting
We can see from all 8 models above that the best model is the support vector machine polynomial model, which yielded the lowest rmse of 94.9. The second best model is the random forest model, which has an rmse of 110, while the worst model was the support vector machine radial basis function with an rmse of 190. 
<br>
We fit the support vector machine polynomial model to the testing set, which ended up yielding a higher rmse of 167, which is still better than most of the models' rmses that were only fit to the training set so far. This phenomenon of the rmse being higher when fit to the testing data is likely because the training data is assessed on data that has been learnt before, while the test dataset may have data that is unknown/not common that may give more errors or misclassification when doing prediction. 
