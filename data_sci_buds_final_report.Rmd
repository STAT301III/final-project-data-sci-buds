...
<br>
We fit the winning model - the support vector machine polynomial model - to the entire training set and then predicted using the test set, which ended up yielding a higher rmse of 167. This is likely because the training data is assessed on data that has been learnt before, while the test dataset may have data that is unknown/not common that may give more errors or misclassification when doing prediction. Although the rmse is higher when fit to the testing set, it is still better than most of the models' rmses that were only fit to the training set so far.
```{r}
